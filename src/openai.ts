...
import { Step, MakeAttachmentSpec, Attachment, User } from "./types";
import {
  RunStep,
...
export class OpenAIAssistantSyncer {
  private NAMESPACE_UUID = "1b671a64-40d5-491e-99b0-da01ff1f3341";

  private openai: OpenAI;
  private api: API;

  constructor(openai: OpenAI, api: API) {
    this.openai = openai;
    this.api = api;
  }
  ...
  async processMessageContent(threadId: string, message: ThreadMessage) {
    ...
    for (const content of message.content) {
      if (content.type === "image_file") {
        ...
        const attachment = await this.api.makeAttachment(
          chainlitThreadId,
  ...
  async syncThread(threadId: string, user?: User) {
    ...
    if (user) {
      const existingUser = await this.api.getUser(user.identifier);
      if (existingUser) {
